...
2. Deterministic core (fs_walk, manifest parsers, AST parsers, repo_mapper/tech_stack_detector nodes, writer templates, store/session_store.py) — no LangGraph yet
3. Real LangGraph orchestration + parallel agents + FastAPI/SSE
4. Verifier + retry loop
5. Q&A (agent_tools.py, create_react_agent, REPL)
6. Polish + PyPI packaging
...
- No database, no Docker dependency for v1 — persistence is plain JSON files under `.code-atlas/state/`.
...
- `store/session_store.py` — the only module that reads/writes `.code-atlas/state/*.json`. Everything else goes through typed accessor calls.
...
Unit tests use a fake chat model (no real API calls). Persistence tests run against plain JSON files in a tmp dir — no mocking or containers needed. See plan.md § "Testing / Verification Plan" for the full breakdown, including the integration fixture repo with a deliberately misleading docstring used to prove the verifier catches false claims.
